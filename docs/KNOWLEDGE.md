...
| Timer-Sharing über Zonenchat: Spieler mit bekanntem Respawn-Timer teilt ihn automatisch per festem, erkennbarem String im Zonenchat (z. B. „Dynamic Encounter respawn in <Zone> in xx:xx" + Prüfsumme über den String, damit er nicht manipuliert/gefälscht werden kann). Eigener Client erkennt diese Zeile per Chat-Hook und übernimmt den Timer automatisch als eigenen — aber NUR, wenn noch kein eigener Timer für diese Zone bekannt ist. Optional (niedrigere Priorität, danach): auch andersartig geteilte/nicht standardisierte Respawn-Angaben erkennen und clever auswerten, dabei aber vor Übernahme aktiv nachfragen (nie automatisch, da nicht durch Prüfsumme abgesichert) — ebenfalls nur wenn kein eigener Timer bekannt ist. Zurückgestellt bis nach Minimode-Umsetzung | Thomas, 2026-07-26, direkt im Anschluss an Minimode-Konzeption genannt | mittel |
